...
params:
  analysis_name: "District 14_Skagit River,Cascade River_fall_salmon_2022_alt"
...
creel$catch <- paste0(
  ...
  utils::URLencode() |>
  readr::read_csv(show_col_types = F) |>
  dplyr::select(interview_id, catch_id, species, life_stage, fate, fish_count) |>
  dplyr::mutate(
    catch_group = paste(species, life_stage, fate, sep = "_") # fish catch groups to estimate catch of
  )
...
stan_data_prelim$interview <- left_join(
  ...
  filter(catch_group == paste(params$species, params$life_stage, params$fate, sep = "_")) |> 
