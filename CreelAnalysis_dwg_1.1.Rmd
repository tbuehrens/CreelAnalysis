...
params:
  analysis_name: "District 14_Skagit River_fall_salmon_2022_alt"
...
creel$catch <- paste0(
  ...
  dplyr::mutate(
    catch_group = paste(species, life_stage 
                        , fin_mark
                        , fate, sep = "_") # fish catch groups to estimate catch of
  )
  ...
  mutate(
    ...
    angler_type = dplyr::case_when( 
        ...
        boat_used == "Yes" & 
          ...
          fish_from_boat == "Bank" ~ "boat" #needed to assign boat anglers that primarily fished from the bank as boat anglers, otherwise assigned NA
        #allow motorboats as "bank": boat_used == "Yes" & !stringr::str_detect(boat_type, "ontoon|ayak") & fish_from_boat == "Bank" ~ "bank"
      ),
    angler_type_ind = as.integer(factor(angler_type)),
...
stan_data_prelim$interview <- left_join(
  ...
  filter(catch_group == paste(params$species, params$life_stage,
                              params$fin_mark,
                              params$fate, sep = "_")) |> 
