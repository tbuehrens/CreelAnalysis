...
# Assign a "Model_Run" number (if model_source == run_new, results will be saved to a new sub-folder; if model_source == load_saved, previous model results will be loaded)
  Model_Run<-9 #Enter numeric number (NOTE: be careful not to over-write previous models runs by entering a number that's already been used)

# Denote which creel model you want to run
  creel_models[,1:3] #model summary table
  model_number<-c(5) # 1 and 2 are the 6-3-2019 uncorr and corr
...
# Specific Stan model arguments
  ...
  adapt_delta<-0.9  # set adapt delta, which is the target average proposal acceptance probability during Stan's adaptation period. Value between 0 and 1. The default is 0.8. Increasing it will force stan to take smaller steps (and thus run time will be longer). 
...
# Results: Summary of Effort, CPUE, and Catch
```{r, out.width="1.0\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Total Catch and Total Effort", "Daily Effort", "Daily Catch", "Daily CPUE"),out.extra=c('page=1', 'page=2','page=3','page=4'),  echo=FALSE}
