...
We will then look at the preseason planned sport fishery impacts relative to the predicted allowable harvest:
...
ggplot(data=PAHdat_plan,aes(x=PAH_plan,y=Exceedence))+
  ...
  xlab("Pre-season Planned % of Allowable (State + Tribal) Co-Manager Harvest")+
...
par(mar=c(5,15,5,5))
names(probs)<-c("best estimate of sport \nfishery impacts as \n % of total (state + tribal) \n allowable","probability sport fishery \n impacts > 50% of total \n (state + tribal) allowable","probability sport fishery \n impacts > 100% of total \n (state + tribal) allowable")
...
Here, we compare the in-season estimated sport fishery catch, assuming 10% C&R mortality,  with the allowable harvest estimated from the forecasted run size and the RMP mortality limits, to estimate the probability that the allowable harvest has been exceeded:
...
ggplot(data=PAHdat,aes(x=PAH,y=Exceedence))+
  ...
  theme_bw()+
  ylab("Probability of Exceedence")+
  xlab("In-season % of Allowable (State + Tribal) Co-Manager Harvest")+
  geom_vline(xintercept=50)+
  geom_vline(xintercept=100,col="red")
```
