...
params:
  analysis_name: "District 13_Stillaguamish River - North Fork_salmon and gamefish_2022-23"
...
creel$event <- paste0(
  ...
  " AND water_body in('", str_replace(input_analysis$water_body, ",|\\|", "','"), "')",
...
creel$catch <- paste0(
  ...
  utils::URLencode() |>
  readr::read_csv(show_col_types = F) |>
  dplyr::select(interview_id, catch_id, species, life_stage, fin_mark, fate, fish_count) |>
  dplyr::mutate(
...
stan_data_prelim$effort_census <- stan_data_prelim$effort_census |> 
  ...
  mutate(
    ...
    angler_type = case_when(
      ...
      stringr::word(count_type, 1) %in% c("Boat","boat") ~ "boat"
    ),
...
stan_data_prelim$effort_index <- stan_data_prelim$effort_index |> 
  ...
  pivot_wider(names_from = count_type, values_from = count_quantity, values_fill = 0) |> # EB added values_fill to assume 0 trailers for NA values 
  arrange(event_date, section, count_sequence)
```
...
stan_data_prelim$interview <- stan_data_prelim$interview |> 
  ...
  mutate(
    ...
    angler_type = dplyr::case_when( 
      ...
      angler_type == "Yes" & str_detect(boat_type, "ontoon|ayak") ~ "bank",
      angler_type == "Yes" & !str_detect(boat_type, "ontoon|ayak")  ~ "boat", #value from boat_used
      fish_from_boat == "Boat" ~ "boat", # new field to parse anglers who use boats into those that mostly fished from boat or mostly fished from shore
    ...
    angler_hours_total = total_group_count * angler_hours) |> 
  filter(angler_hours_total >= 0.5)




stan_data_prelim$interview <- left_join(
  stan_data_prelim$interview 
  # |> 
  ...
  pivot_longer(cols = -c(creel_event_id:angler_hours_total), names_to = "catch_group", values_to = "fish_count") |> 
  mutate(fish_count = replace_na(fish_count, 0)) |>
  filter(catch_group == paste(params$species, params$life_stage, params$fin_mark, params$fate, sep = "_")) |> 
...
## TO ALLOW RAGGED VEHICLE/TRAILER VECTORS 
  ...
  gear_T = as.integer(rep(as.integer(length(unique(stan_data_prelim$interview$angler_type))), nrow(stan_data_prelim$effort_index))), #SAME - TEMP OPTION PENDING OTHER CHANGES
  # int; index for section
  ...
  # Census (tie-in) effort counts   
	...
	E_n = nrow(stan_data_prelim$effort_census%>%filter(angler_type=="bank")),
  # int; index denoting day/period
	day_E = stan_data_prelim$effort_census%>%filter(angler_type=="bank")%>%dplyr::select(day_index)%>%extract_numeric(),
  # int; index denoting "gear/angler type"  
  gear_E = stan_data_prelim$effort_census%>%filter(angler_type=="bank")%>%dplyr::select(angler_type_ind)%>%extract_numeric(), #if_else(stan_data_prelim$effort_census$angler_type == "Boat", 2, 1),
  # int; index for section
  section_E = stan_data_prelim$effort_census%>%filter(angler_type=="bank")%>%dplyr::select(section)%>%extract_numeric(),
	# EB count_sequence here needs to match that of the closest effort_index count (if my understanding is correct)
  # int; index for count_num
  countnum_E = stan_data_prelim$effort_census%>%filter(angler_type=="bank")%>%dplyr::select(count_sequence)%>%extract_numeric(), 
  # int; observed # of anglers
  E_s = stan_data_prelim$effort_census%>%filter(angler_type=="bank")%>%dplyr::select(count_quantity)%>%extract_numeric(),
  ...
  value_normal_mu_mu_C = log(sum(stan_data_prelim$interview$fish_count)/sum(stan_data_prelim$interview$angler_hours_total)), #the mean hyperparameter in the prior distribution mu_C; median (log-space); default = 0.02 (was originally  0.05) 
...
if(!file.exists(file.path("results",paste0(results_name,".rds")))){
  stan_fit <- stan(
    file = "models/BSS_creel_model_09_2022-09-30_1G_1S.stan",
    data = stan_data,
    chains = n_chain,
    cores = n_cores,
    iter = n_iter,
...
results$e_sum_c_sum |>
  ...
  select(estimate, `2.5%`:`97.5%`, mean, sd, cv) |>
  gt() |>
