...
transformed parameters{
	...
	for(g in 1:G){
		...
		for(d in 2:D){
			eps_C[d,g] = phi_C * eps_C[d-1,g] + eps_C_eps[d-1,g]; //eps_C[d,g] = eps_C[d-1,g] + eps_C_eps[d-1,g];//RW
		...
		for(d in 1:D){
			...
			//lambda_C[d,g] = exp(mu_C_mu[g] + eps_C[d,g]) * O[d,1];// + B2 * w[d]); //lambda_C[d,g] = exp(eps_C[d,g] + B2 * w[d]);//RW
			...
			for(s in 1:S){	
				lambda_E_S[s][d,g] = lambda_E[d,g] * p_S[s][d,g]; //new line of code (Kale)
				lambda_C[d,g] = exp(mu_C_mu[g] + eps_C[d,g]) * O[d,s];										
				for(i in 1:H){
...
model{
	...
	for(g in 1:G){
		for(d in 2:D){
			eps_C_eps[d-1,g] ~ normal(0,sigma_C);
...
/*		for(W in 2:Wk_n){
		...
		eps_C_0[g] ~ normal(0,0.4); //eps_C_0[g] ~ normal(-2, 0.5);//RandomWalk
	...
	//Angler Likelihood
	for(a in 1:A){
		c[a] ~ neg_binomial_2(lambda_C[day_A[a],gear_A[a]] * h[a] , phi_NB_C);
...
generated quantities{
	matrix<lower=0>[D,G] C;
	matrix<lower=0>[D,G] E[S];//matrix<lower=0>[D,G] E;
	real<lower=0> C_sum;
	real<lower=0> E_sum;
	...
	for(g in 1:G){
		for(d in 1:D){
			C[d,g] = poisson_rng(lambda_E[d,g] * L[d] * lambda_C[d,g]); 
			C_sum += C[d,g];
			//E[d,g] = poisson_rng(lambda_E[d,g] * L[d]); 
			//E_sum += E[d,g];
			for(s in 1:S){
	...
	//Angler Likelihoods
	for(a in 1: A){
		log_lik[V_n + T_n + E_n + a] = neg_binomial_2_lpmf(c[a]|lambda_C[day_A[a],gear_A[a]] * h[a] , phi_NB_C);
	}
	for(a in 1: A){
