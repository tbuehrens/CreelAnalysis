...
parameters{
	//Effort
	matrix[G,S] mu_E_mu; //season-long effort intercept  
	...
	real<lower=0> sigma_E_H; //standard deviation for within day variability in observed effort counts
	real<lower=0,upper=1> phi_E_prior; //prior on a transformation of phi_E								    			
	matrix[P_n-1,G] eps_E_eps[S]; //effort process errors  
	matrix[G,S] eps_E_0; //effort residual for initial time step (p=1)
	...
	//Catch rates
	matrix[G,S] mu_C_mu; //season-long catch rate intercept                            							
	...
	real<lower=0> sigma_C_A; //Prior on r_C
	matrix[P_n-1,G] eps_C_eps[S]; //CPUE process errors 
	matrix[G,S] eps_C_0; //CPUE residual for initial time step (p=1)                             							
}
transformed parameters{
	//Effort
	real<lower=-1,upper=1> phi_E; //auto-regressive (AR), mean-reverting lag-1 coefficient for effort 
	matrix[P_n,G] eps_E[S]; //residual in effort 
	...
	matrix[P_n,G] eps_C[S]; //Residual in CPUE
	matrix<lower=0>[D,G] lambda_C_S[S]; //mean daily CPUE

	r_E = 1 / square(sigma_E_H);
	r_C = 1 / square(sigma_C_A);
	...
	for(g in 1:G){
		...
		for(p in 2:P_n){ 
			for(s in 1:S){
				eps_C[s][p,g] = phi_C * eps_C[s][p-1,g] + eps_C_eps[s][p-1,g]; 
				eps_E[s][p,g] = phi_E * eps_E[s][p-1,g] + eps_E_eps[s][p-1,g]; 
			}													
		}
		for(d in 1:D){       
			for(s in 1:S){	
				lambda_C_S[s][d,g] = exp(mu_C_mu[g,s] + eps_C[s][period[d],g]) * O[d,s];
				lambda_E_S[s][d,g] = exp(mu_E_mu[g,s] + eps_E[s][period[d],g] + B1 * w[d])* O[d,s];
				for(i in 1:H){
...
model{
	...
	sigma_C_A ~ cauchy(0, prior_r_C); 
	for(g in 1:G){
		for(p in 2:P_n){ 
			for(s in 1:S){
				eps_C_eps[s][p-1,g] ~ normal(0,sigma_C);
				eps_E_eps[s][p-1,g] ~ normal(0,sigma_E);
			}
		}
		for(d in 1:D){
			for(s in 1:S){  					  
