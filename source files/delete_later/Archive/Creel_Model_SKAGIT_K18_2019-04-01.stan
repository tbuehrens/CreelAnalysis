...
parameters{
	...
	matrix[P_n-1,G] eps_E_eps[S];  //*************** used to be D-1 (instead of P_n -1)
	matrix[G,S] eps_E_0;
	...
	matrix[P_n-1,G] eps_C_eps[S];  //*************** used to be D-1 (instead of P_n -1)
	matrix[G,S] eps_C_0;                              							
}
transformed parameters{
	...
	for(g in 1:G){
		...
		for(p in 2:P_n){ //*************** p instead of d
			for(s in 1:S){
				eps_C[s][p,g] = phi_C * eps_C[s][p-1,g] + eps_C_eps[s][p-1,g]; //*************** p instead of d
				eps_E[s][p,g] = phi_E * eps_E[s][p-1,g] + eps_E_eps[s][p-1,g]; //*************** p instead of d
			}													
		}
		for(d in 1:D){       
			for(s in 1:S){	
