...
#---------------------------------------------------------------------------------------------------------- -    
    ...
    #     day.light<-getSunlightTimes(keep=c("dawn", "dusk"), date=seq(min(as.Date(sub.header.dat$Date)), max(as.Date(sub.header.dat$Date)),1), lat = river.loc$Lat[river.loc$River == StreamName.of.Interest], lon=river.loc$Long[river.loc$River == StreamName.of.Interest], tz = "America/Los_Angeles")
    ...
    #Define "Section.Name" and "Section.Num" based on cross-walk table
      #Effort data  
        ...
        for(row in 1:nrow(sub.effort.dat)){
          sub.effort.dat$final.Section.Name[row]<-as.character(effort.xwalk$Section.Name[as.character(effort.xwalk$Section.Field) == as.character(sub.effort.dat$SectionName[row]) & as.character(effort.xwalk$YearGroup) == as.character(sub.effort.dat$YearGroup[row])])
          sub.effort.dat$Section.Num[row]<-effort.xwalk$Section.Summ[as.character(effort.xwalk$Section.Field) == as.character(sub.effort.dat$SectionName[row]) & as.character(effort.xwalk$YearGroup) == as.character(sub.effort.dat$YearGroup[row])]
        }  
      ...
      #Interview data
        ...
        for(row in 1:nrow(sub.group.dat)){
          sub.group.dat$final.Section.Name[row]<-as.character(effort.xwalk$Section.Name[as.character(effort.xwalk$Section.Field) == as.character(sub.group.dat$SectionName[row]) & as.character(effort.xwalk$YearGroup) == as.character(sub.group.dat$YearGroup[row])])
          sub.group.dat$Section.Num[row]<-effort.xwalk$Section.Summ[as.character(effort.xwalk$Section.Field) == as.character(sub.group.dat$SectionName[row]) & as.character(effort.xwalk$YearGroup) == as.character(sub.group.dat$YearGroup[row])]
        }  

    #Create index effort x-walk table (that will help define "outage" dates by section in "master" creel analysis file)
          sub.river.effort.xwalk<-effort.xwalk[effort.xwalk$StreamName %in% unique(sub.effort.dat$StreamName), ]
    ...
    #Calculate "Hours" fished by Group
        ...
        if(any(colnames(sub.group.dat)=="Percent_time_fished")==TRUE){
          sub.group.dat$Trip.Status[row]<-sub.group.dat$End.Time.Dec[row] - sub.group.dat$Start.Time.Dec[row]
