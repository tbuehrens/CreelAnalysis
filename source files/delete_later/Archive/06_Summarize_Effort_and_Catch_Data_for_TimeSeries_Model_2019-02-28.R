...
#---------------------------------------------------------------------------------------------------------- - 
  #Create cross-walk table "all.Dates" that defines the relative "day" and "week" number based on "Date_Begin" and "Date_End"
      ...
      all.Dates$DayofWeek<-weekdays(as.Date(all.Dates$Date)); all.Dates$DayType<-ifelse(all.Dates$DayofWeek=="Saturday"|all.Dates$DayofWeek=="Sunday",1,0)
  ...
  ##Day Length
      ...
      day.light<-getSunlightTimes(keep=c("dawn", "dusk"), date=seq(min(as.Date(all.Dates$Date)), max(as.Date(all.Dates$Date)),1), lat = river.loc$Lat[river.loc$River == StreamName.of.Interest], lon=river.loc$Long[river.loc$River == StreamName.of.Interest], tz = "America/Los_Angeles")
...
#---------------------------------------------------------------------------------------------------------- - 
  ...
  #Drop "final_group.dat" data if "Start.Time" is <NA>, "Interview.Time" is <NA> for groups with Trip.Status == I, or "End.Time" is <NA> for groups with Trip.Status == C
    drop.group.indices<-c(final_group.dat$GroupNum[is.na(final_group.dat$Start.Time)==TRUE] #Missing Start Time
      ...
      , final_group.dat$GroupNum[is.na(final_group.dat$Start.Time)==FALSE & is.na(final_group.dat$End.Time)==TRUE & final_group.dat$Trip.Status=="C" ] #Missing Interview Time for Completed Trips
      )
  ...
  #Summarize group level data by "Date" and "GroupNum"
    for(row in 1:nrow(final_group.dat)){final_group.dat$temp.Group[row]<-paste(final_group.dat$Date[row], final_group.dat$GroupNum[row], sep="_")}
  ...
  #Identify catch groupings of interest
    ...
    #catch.group.of.interest<-c("SH_NOR_R", "SH_HOR_K", "SH_HOR_R")
  ...
  #Catch samples
    c_samp<-final_Group_Catch.dat[, c("Day", "DayType", "Week", "Gear", "Angler", "Section.Num", "NumAnglers", "Vehicles", "Trailers", "Total_Hours", catch.group.of.interest)]
    head(c_samp)
...
standat<-list(
  ...
  #angler data
  A=nrow(c_samp),              # total number of interviews
  gear_A=c_samp$Gear,          # an indexing column with length equal to count of angler interviews with a 	1 or 2 depending on whether the interview was a boat or bank fisherman
  day_A=c_samp$Day,            # an indexing column with length equal to count angler interviews with the day of the fishery listed starting at 1 and going to the last day n.
  section_A = c_samp$Section.Num, #Section where angler was fishing (1 = Skagit, 2 = Sauk)
  ...
  V_A= c_samp$Vehicles,        # Total number of vehicles an individual angler group brought to the river
  T_A= c_samp$Trailers,        # Total number of Trailers an individual angler group brought to the river
  A_A= c_samp$NumAnglers,       #Total number of anglers in each group interviewed

  #priors
  prior_sigma_C = 1, #cauchy df 
  prior_sigma_E = 1, #cauchy df 
  prior_sigma_E_H = 1, #cauchy df 
  prior_B1 = 5, #normal sd (log-space)
  prior_phi_NB_C = 1, #cauchy df 
  prior_eps_C_0 = 1,#1 #normal sd (log-space)
  prior_eps_E_0 =  3,#1 #normal sd (log-space)
  prior_b = 1, #log normal sd
  prior_mu_C_mu_mu = log(0.05), #median (log-space)
  prior_mu_C_mu_sd = 5,#1.5 #normal sd (log-space)
  prior_mu_E_mu_mu = log(15), #median (logspace)
  prior_mu_E_mu_sd = 5,#2 #normal sd (log-space)
  prior_phi_E_prior = 1, #beta(1,1) or beta(2,2); 1,1 is uniform
  prior_phi_C_prior = 1 #beta(1,1) or beta(2,2); 1,1 is uniform
)
