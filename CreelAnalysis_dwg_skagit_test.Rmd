...
params:
  ...
  sections: "lut_water_body_location_d14_skagit_winter_gf.csv"
  species: "Steelhead"
  fin_mark: "UM"
  fate: "Released"
  model_number: 4
...
lut <- map(
  list(
    ...
    census_expansion = "input_files/lut_Proportional_Expansions_for_Tie_In_Sections_Skagit_WGF_2022.csv"
...
## one-off clean up stuff

```{r data_issues}
#Skagit winter game fish data reveal lack of generalizability in prior wrangling code

#no `Trailer only` counts for section; okay to exclude due to very low effort
#also no census records for section 1: count(stan_data_prelim$effort_census, section)
#unfortunately need to decrement in order to satisfy stan indexing starting at 1
stan_data_prelim$effort_census <- filter(stan_data_prelim$effort_census, section > 1) |> 
  mutate(section = section - 1)
stan_data_prelim$effort_index <- filter(stan_data_prelim$effort_index, section > 1) |> 
  mutate(section = section - 1)
stan_data_prelim$interview <- filter(stan_data_prelim$interview, section > 1) |> 
  mutate(section = section - 1)

map(stan_data_prelim, ~count(.x, section))

#also need to remove from prop expansion LU
lut$census_expansion <- filter(lut$census_expansion, section > 1) |> 
  mutate(section = section - 1)

lut$census_expansion |> count(section)

#note this means stan output sections need to be +1 to join against original LUs
```
...
## where vehicle==1 & trailer==2, then bank==1 & boat==2
  gear_E = if_else(stan_data_prelim$effort_census$angler_type == "Boat", 2, 1),
  # int; index for section
  ...
  E_s = stan_data_prelim$effort_census$count_quantity,
	...
	# int; index denoting "gear/angler type" 
	# EB: depending on the variety of angler types (eg multiple types of boats) this may not be flexible enough for other datasets 
	gear_IntC = if_else(stan_data_prelim$interview$angler_type == "Boat", 2, 1),
	# int; index for section
  ...
  # EB Does Total_Hours refer to fishing time (angler hours) multiplied by the total number of anglers in an                    interviewed party (group_angler_hours)? 
	...
	# int; index denoting "gear/angler type"  
# EB: depending on the variety of angler types (eg multiple types of boats) this may not be flexible enough for other datasets 
	gear_Creel = if_else(stan_data_prelim$interview_daily_totals$angler_type == "Boat", 2, 1),
	# int; index for section 
  ...
  # EB: depending on the variety of angler types (eg multiple types of boats) this may not be flexible enough for other datasets 
	gear_IntA = if_else(stan_data_prelim$interview$angler_type == "Boat", 2, 1),
	# int; index denoting day/period
...
summary(stan_fit, pars = c("E_sum", "C_sum"))$summary
...
#summary(stan_fit, pars = c("C"))$summary |> 
  as.data.frame() |> rownames_to_column("stan_out") |> tibble() |>
  mutate(
    inds = str_sub(stan_out, 3, 20) |> str_remove("\\]")
  ) |> 
  separate(col = inds, into = c("section","day","gear")) |> 
  #count(section, gear)
  #could also mutate this...
  left_join(
    distinct(creel$effort,  section) |> mutate(section = as.character(section)),
    by = "section"
    ) |> 
  #Bank? inverts if_else calls in stan_data declaration
  mutate(
    angler_type = if_else(gear == "2", "Boat", "Shore"),
    day = as.numeric(day)
  ) |> 
  select(day, section, water_body, gear, angler_type, `2.5%`, `50%`, `97.5%`) |> 
  ggplot(aes(x = day, y = `50%`, fill = angler_type, color = angler_type)) +
  geom_ribbon(aes(ymin = `2.5%`, ymax = `97.5%`), alpha = 0.5) +
  geom_line() + geom_point() +
  facet_wrap(~water_body, ncol = 1)
  


# distinct(creel$effort, section, water_body)
# distinct(creel$interview, section, water_body)

distinct(stan_data_prelim$effort_index, angler)
stan_data_prelim$interview

#grep("^E", names(stan_fit), value = T)
