...
##Sure the left_join of filtered catches to the full interview set is what we want?
##creates a ton of NA fish_counts (as might be expected if many interviews do not report catch values fitting our params filter conditions)
  interview_catch = left_join(
    creel$interview
    ,
    creel$catch |> 
      #replacing: #tidyr::unite(col = "species_fin_mark_fate", c(species, fin_mark, fate)) |> filter(species_fin_mark_fate == params$catch_group)
      filter(
        str_detect(species, params$species),
        str_detect(fin_mark, params$fin_mark),
        str_detect(fate, params$fate)
      )
    ,
    by = "interview_id"
  )
...
# #is this still necessary? does not seem to be...I see no NAs at least in these data...
...
# re-index sections for stan_dat list
#still necessary/appropriate?
    section_dense = dense_rank(section)
  ) |>
#  count(section, section_dense)
  group_by(event_date, day_index, section, section_dense, count_sequence, angler_type) |> # aggregate census counts to bank / boat categories
...
##DA added filter() after select() as quick workaround to above comments...
...
##not a good long term solution...
  ...
  mutate(
##not seeing any NAs and don't want to do this if not strictly needed
    #across(-c(section:count_sequence), ~replace_na(., 0)),
    section_dense = dense_rank(section)
    ) |>
  arrange(event_date, section, count_sequence)
```
...
```{r}
#previously object named "interview_index"
stan_data$interview_catch <- stan_data$interview_catch |> 
  ...
  mutate(
    ...
    across(c(vehicle_count, trailer_count, fish_count), ~replace_na(., 0)),
...
## DA START HERE - not clear we want a group_by that includes vehicle_count, trailer_count, etc. if interview_id is UID...
...
## can delete?
# catch_cg <- creel$catch |> # catch table filtered to catch group (cg) parameter 
#   mutate(
#     origin = case_when(
#       fin_mark == "UM" ~ "W",
#       fin_mark == "AD" ~ "H",
...
```{r}
stan_data$effort_census |> count(section, section_dense)
stan_data$effort_index |> count(section, section_dense)
stan_data$interview_catch |> count(section, section_dense)
```
