...
#---------------------------------------------------------------------------------------------------------- - 
  ...
  #Calculate relative day (DayNum_Rel) and week (WeekNum_Rel) columns for "Dates.of.Interest" DF
      ...
      daynum_xwalk<-setNames(as.data.frame(cbind(daynums.abs, daynum.rel)), c("Date", "daynum"))
      for(row in 1:nrow(all.Dates)){all.Dates$Daynum_Rel[row]<-as.numeric(daynum_xwalk$daynum[as.character(daynum_xwalk$Date) == as.character(all.Dates$Date)[row]])}
      head(all.Dates)
