...
get_creel <- function(
  water_body, #test against 'water_body_desc'
  date_start, #test against 'event_date'
  date_end, #default could be Sys.Date()?
  proj_name # test against 'project_name'
  ){
  date_start <- as.Date(date_start); date_end <- as.Date(date_end)
  
  event = tbl(con_creel, dbplyr::in_schema("creel","vw_steelhead_creel_event")) %>%
    filter(
      str_detect(water_body_desc, water_body),
      str_detect(project_name, proj_name),
      between(event_date, date_start, date_end)
    )
  
  effort = event %>% left_join(
    tbl(con_creel, dbplyr::in_schema("creel", "vw_steelhead_effort_count")) 
    , by = "creel_event_id", suffix = c("_event", "_effort"))
  
  interview = event %>% left_join(
    tbl(con_creel, dbplyr::in_schema("creel", "vw_steelhead_interview")) 
    , by = "creel_event_id", suffix = c("_event", "_interview"))

  catch = interview %>% left_join(
    tbl(con_creel, dbplyr::in_schema("creel","vw_steelhead_catch"))
...
con_creel <- DBI::dbConnect(odbc::odbc(), Driver = "PostgreSQL Unicode", Server = "35.167.13.195", Database = "FISH", UID = u, PWD = p, Trusted_Connection = "True")

#example single pull
(z <- get_creel(
  water_body = analysis_args$water_body,
  date_start = analysis_args$date_start,
  date_end = analysis_args$date_end,
  proj_name = analysis_args$proj_name
  ))

# #example pull same month across different years...
# z2 <- c(2015:2017) %>% set_names() %>%
#   map(
#     ~get_creel(
#       water_body = analysis_args$water_body, proj_name = analysis_args$proj_name,
#       date_start = paste0(.x, "-03-01"), date_end = paste0(.x, "-03-31")
#       )
#     )

# #needs to join on creel_event_id + interview_id?
# tbl(con_creel, dbplyr::in_schema("creel", "vw_steelhead_gear_hook"))
...
df_header<-
  z %>% 
  `[[`("event") %>% 
  select(creel_event_id, water_body_desc, event_date, tie_in_indicator) 

df_effort<-
  z$effort %>% 
  select(creel_event_id, effort_event_id, tie_in_indicator, location, count_sequence, start_time_effort, end_time_effort, boat_anglers, bank_anglers) # will want option to select "angler type" fields; may also want counts by angler type summarized by row instead of column


df_interview<-
  z %>% 
  `[[`("interview") %>% 
  select(creel_event_id, interview_id, interview_number, angler_type_code, interview_location, angler_count, trip_status, fishing_start_time, fishing_end_time, interview_time, ) # Noticed there are locations listed as NA

df_catch<-
  z %>% 
  `[[`("catch") %>% 
  select(creel_event_id, interview_id, catch_id, species_name, run_name, life_stage_name, fin_mark_code, fate_name, fish_count) 
...
standat<-
  list(
# Day attributes
	  D =            # int; number of fishing days;                                KB: pull from master date DF
	...
	, P_n =          # int; #number of periods (number of states for our state variable); P_n can equal D or some other interval (e.g., weekly)
  , w=             # vec; index denoting daytype, where 0=weekday, 1= weekend/holiday.  
	, period =       # int; index denoting period                                  KB: not sure why this isn't a vector)   
	, L =            # vec; total amount of available fishing hours per day (e.g., day length - sunrise to sunset)
	, O =            # mat; index denoting fishery status, where 1=open, 0 = closed (by period/date and section)
	# Vehicle index effort counts
	, V_n =          # int; total number of individual vehicle index effort counts
	, day_V =        # int; index denoting the "day" for an individual vehicle index effort count
	, section_V =    # int; index denoting the "section" for an individual vehicle index effort count
	, countnum_V =   # int; index denoting the "count number" for an individual vehicle index effort count
	, V_I =          # int; number of vehicles enumerated during an individual index effort survey
	# Trailer index effort counts 
	, T_n =          # int; total number of boat trailer index effort counts
	, day_T =        # int; index denoting the "day" for an individual boat trailer index effort count
	, section_T =    # int; index denoting the "section" for an individual boat trailer index effort count
	, countnum_T =   # int; index denoting the "count number" for an individual boat trailer index effort count
	, T_I =          # int; number of boat trailers enumerated during an individual index effort survey
	# Angler index effort counts
	, A_n =           # int; total number of angler index effort counts             
	, day_A =         # int; index denoting the "day" for an individual angler index effort count     
	, gear_A =        # int; index denoting the "gear/angler type" for an individual angler index effort count (e.g., 1=bank and 2=boat)     
	, section_A =     # int; index denoting the "section" for an individual angler index effort count  
	, countnum_A =    # int; index denoting the "count number" for an individual angler index effort count
	, A_I =           # int; number of anglers enumerated during an individual index effort survey         
	# Census (tie-in) effort counts
	, E_n =           # int; total number of angler census effort counts 
	, day_E =         # int; index denoting the "day" for an individual angler census effort count
	, gear_E =        # int; index denoting the "gear/angler type" for an individual angler census effort count (e.g., 1=bank and 2=boat) 
	, section_E =     # int; index denoting the "section" for an individual angler census effort count  
	, countnum_E =    # int; index denoting the "count number" for an individual angler census effort count
	, E_s =           # int; number of anglers enumerated during an individual census effort survey
	# Proportion tie-in expansion
	, p_TI =          # mat; proportion of section covered by tie in counts (serves as an expansion if p_TI != 1 by gear and section
	# interview data - CPUE
	, IntC =          # int; total number of angler interviews conducted across all surveys dates where CPUE data (c & h) were collected                                       	
	, day_IntC =      # int; index denoting the "day" for an individual angler interview                       	
	, gear_IntC =     # int; index denoting the "gear/angler type" for an individual angler interview (e.g., 1=bank and 2=boat)      						
	, section_IntC =  # int; index denoting the "section" for an individual angler interview    						
	, c =             # int; total number of fish caught by an angler (group) collected from an individual angler interview           						
	, h =             # vec; total number of hours fish by an angler (group) collected from an individual angler interview
	...
	, IntA =          # int; total number of angler interviews conducted across all surveys dates where angler expansion data (V_A, T_A, A_A) were collected 
	, day_IntA =      # int; index denoting the "day" for an individual angler interview                       	
	, gear_IntA =     # int; index denoting the "gear/angler type" for an individual angler interview (e.g., 1=bank and 2=boat)      						
	, section_IntA =  # int; index denoting the "section" for an individual angler interview    
	, V_A =           # int; total number of vehicles by an individual angler/group brought to the fishery on a given survey date		
	, T_A =           # int; total number of boat trailers by an individual angler/group brought to the fishery on a given survey date			
	, A_A =           # int; total number of anglers in the group interviewed 
	# hyper and hyperhyper parameters (add back in later)
