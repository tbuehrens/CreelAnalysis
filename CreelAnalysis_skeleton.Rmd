...
get_creel <- function(
  ...
  event = tbl(con_creel, dbplyr::in_schema("creel","vw_analysis_creel_event")) %>%
    filter(
      str_detect(water_body, water_body),
      str_detect(project_name, proj_name),
      between(event_date, date_start, date_end)
    )
  
  effort = event %>% left_join(
    tbl(con_creel, dbplyr::in_schema("creel", "vw_analysis_effort_counts"))
    , by = "creel_event_id", suffix = c("_event", "_effort"))

  interview = event %>% left_join(
    tbl(con_creel, dbplyr::in_schema("creel", "vw_analysis_interview"))
    , by = "creel_event_id", suffix = c("_event", "_interview"))

  catch = interview %>% left_join(
    tbl(con_creel, dbplyr::in_schema("creel","vw_analysis_catch"))
    , by = c("creel_event_id", "interview_id"), suffix = c("_interview", "_catch")
    )
    
  #d <- list(event = event) %>% map(~collect(.x))
