...
```{r kale test arena}
df_header <- creel_data$event %>% 
  dplyr::select(creel_event_id, water_body_desc, event_date, tie_in_indicator) 

# will want option to select "angler type" fields; may also want counts by angler type summarized by row instead of column
df_effort <- creel_data$effort %>%
  dplyr::select(creel_event_id, effort_event_id, tie_in_indicator,
                location, count_sequence, start_time_effort, end_time_effort,
                boat_anglers, bank_anglers) 

# Noticed there are locations listed as NA
df_interview <- creel_data$interview %>%
  dplyr::select(creel_event_id, interview_id, interview_number, 
                angler_type_code, interview_location,
                angler_count, vehicle_count, trailer_count,
                trip_status, fishing_start_time, fishing_end_time, interview_time, boat_type, fish_from_boat)
  
df_catch <- creel_data$catch %>%
  dplyr::select(creel_event_id, interview_id, catch_id, 
                species_name, run_name, life_stage_name, fin_mark_code, fate_name, fish_count)
...
creel_data$effort %>%
  group_by(tie_in_indicator, location, count_sequence) %>%
  summarise(across(c(boat_anglers, bank_anglers), ~sum(., na.rm = T)), .groups = "drop")
