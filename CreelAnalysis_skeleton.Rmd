...
Modify to accommodate creel database and/or older loose files? Seems preferable to get older loose data into the database - is that likely?
...
(closure_dates <- bind_rows(
  ...
  tibble(closure_begin = "2021-02-10", closure_end = "2021-02-11", section = "1,2")
  ) %>%
  rowwise() %>%
...
#such than objects are declared responsively/reactively
...
d_effort <- d_effort %>% 
  left_join(
    d_effort %>% distinct(Date, Survey.Type, Effort_StartTime) %>% rowid_to_column(var = "HeaderID"),
    by = c("Date", "Survey.Type", "Effort_StartTime")
  ) %>%
  select(
    Effort.Index, HeaderID, 
    StreamName, Date, Surveyor, Year, 
    #YearGroup, Season,
    Month, Month.no, Weeknum, j.date, Day, DayType,
...
d_header <- d_effort %>%
  select(StreamName, Date, Year, 
         #YearGroup, Season, 
         Month, Month.no, Weeknum, j.date, Day, DayType, Survey.Type, Surveyor, HeaderID) %>%
  distinct(HeaderID, .keep_all = T)

```

```{r data_prep_group}
#left as mostly duplicative with d_effort for now but needs confer with KB
...
d_group <- readr::read_csv("C:/Users/auerbdaa/Downloads/Interview_dat_2021_steelhead_cleaned_04132021.csv") %>%
  ...
  mutate(
    ...
    across(c(Fishing_Start_Time, Interview_Start_Time), as.character),
    across(c(Fishing_Start_Time, Interview_Start_Time), .fns = list(Dec = function(x) {round(as.numeric(chron::times(x))*24, 3)})),
...
# #DAA: this is a little unclear...need to confer with KB
...
DA will delete this chunk

```{r extract, message=FALSE, warning=FALSE,results = "hide",echo=FALSE}
# # Extract data of interest and format, code that shows options for filtering data by date/year/season/location
...
standat <- list(
  ...
  # #appears this could derive from closure_dates (i.e., shown above)
  # #in conjuction with date handling via d_header or finalized d_effort
