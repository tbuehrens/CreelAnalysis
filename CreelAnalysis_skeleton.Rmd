...
# Extract dataframes from list "z"
  df_header<-
    z %>% 
    `[[`("event") %>% 
  ...
  df_effort<-
    z$effort %>% 
    dplyr::select(creel_event_id, effort_event_id, tie_in_indicator, location, count_sequence, start_time_effort, end_time_effort, boat_anglers, bank_anglers) # will want option to select "angler type" fields; may also want counts by angler type summarized by row instead of column
  
  
  df_interview<-
    z %>% 
    `[[`("interview") %>% 
    dplyr::select(creel_event_id, interview_id, interview_number, angler_type_code, interview_location, angler_count, vehicle_count, trailer_count, trip_status, fishing_start_time, fishing_end_time, interview_time, boat_type, fish_from_boat) # Noticed there are locations listed as NA
  
  df_catch<-
    z %>% 
    `[[`("catch") %>% 
    dplyr::select(creel_event_id, interview_id, catch_id, species_name, run_name, life_stage_name, fin_mark_code, fate_name, fish_count) 
