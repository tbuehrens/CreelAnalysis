...
effort <- effort %>% 
  left_join(
    effort %>% distinct(Date, Survey.Type, Effort_StartTime) %>% rowid_to_column(var = "HeaderID"),
    by = c("Date", "Survey.Type", "Effort_StartTime")
  ) %>%
  select(
    Effort.Index, HeaderID, 
    StreamName, Date, Surveyor, Year, YearGroup, Season, Month, Month.no, Weeknum, j.date, Day, DayType,
