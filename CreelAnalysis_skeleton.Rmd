...
#can/should/will redo this after talking to Kale and Danny
get_creel <- function(river = "Kalama", date_start = "2019-03-01", date_end = "2019-03-31"){
  ...
  event = tbl(con_creel, dbplyr::in_schema("creel","vw_steelhead_creel_event")) %>%
    filter(
      str_detect(water_body_desc, river),
  ...
  d <- list(event = event, effort = effort, interview = interview) %>% map(~collect(.x))
