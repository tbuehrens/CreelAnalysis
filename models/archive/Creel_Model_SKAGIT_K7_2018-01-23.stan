...
parameters{
	...
	matrix<lower=0,upper=1>[G,S] R_V; //true angler vehicles per angler
	matrix<lower=0,upper=1>[G,S] R_T; //true angler trailers per angler
	matrix<lower=0>[G,S] b; //bias in angler vehicles per angler from road counts of cars
...
/*		for(W in 2:Wk_n){
		...
		for(s in 1:(S-1)){
			...
			mu_P[g,s] ~ normal(0,5); //may need to tweak
		}
		for(s in 1:S){
			...
			b[g,s] ~ gamma(1E-06,1E-06);
			R_V[g,s] ~ beta(0.5,0.5); //leaving constant among days AND sections...may need to tweak; can make beta because is "true" angler cars or angler trailers per angler!
			R_T[g,s] ~ beta(0.5,0.5); //leaving constant among days AND sections...may need to tweak; can make beta because is "true" angler cars or angler trailers per angler!
		}
		...
		//b[g] ~ gamma(1E-06,1E-06); //leaving constant among days AND sections...may need to tweak could go as low as 0.25 for sigma
	}
	...
	for(i in 1:V_n){
		...
		V_I[i] ~ poisson((lambda_E_S_I[section_V[i],countnum_V[i]][day_V[i],1] * p_TE[1,section_V[i]] * R_V[1,section_V[i]] +
						 lambda_E_S_I[section_V[i],countnum_V[i]][day_V[i],2] * p_TE[2,section_V[i]] * R_V[2,section_V[i]]) * b[1,section_V[i]]);  //Note: leaving ratio of cars per angler and bias constant among days since was invariant!
	}
	for(i in 1:T_n){
		T_I[i] ~ poisson((lambda_E_S_I[section_T[i],countnum_T[i]][day_T[i],1] * p_TE[1,section_T[i]] * R_T[1,section_T[i]] +
						 lambda_E_S_I[section_T[i],countnum_T[i]][day_T[i],2] * p_TE[2,section_T[i]] * R_T[2,section_T[i]]) * b[2,section_T[i]]); 
	}
	...
	for(a in 1:A){
		...
		V_A[a] ~ binomial(A_A[a], R_V[gear_A[a],section_A[a]]);  //Note: leaving ratio of cars per angler constant among days since was invariant!
		T_A[a] ~ binomial(A_A[a], R_T[gear_A[a],section_A[a]]);  //Note: leaving ratio of cars per angler constant among days since was invariant!
	}												
}
generated quantities{
	...
	for (i in 1:V_n){
		log_lik[i] = poisson_lpmf(V_I[i]|(lambda_E_S_I[section_V[i],countnum_V[i]][day_V[i],1] * p_TE[1,section_V[i]] * R_V[1,section_V[i]] +
						 				  lambda_E_S_I[section_V[i],countnum_V[i]][day_V[i],2] * p_TE[2,section_V[i]] * R_V[2,section_V[i]]) * b[1,section_V[i]]);
	}
	for(i in 1:T_n){
		log_lik[V_n +i] = poisson_lpmf(T_I[i]|(lambda_E_S_I[section_T[i],countnum_T[i]][day_T[i],1] * p_TE[1,section_T[i]] * R_T[1,section_T[i]]  +
						                       lambda_E_S_I[section_T[i],countnum_T[i]][day_T[i],2] * p_TE[2,section_T[i]] * R_T[2,section_T[i]]) * b[2,section_T[i]]);
	}
	//Tie in count Likelihood
	for(e in 1:E_n){
	...
	for(a in 1: A){
		log_lik[V_n + T_n + E_n + A + a] = binomial_lpmf(V_A[a]|A_A[a], R_V[gear_A[a],section_A[a]]); //Note: leaving ratio of cars per angler constant among days since was invariant!
	}
	for(a in 1: A){
		log_lik[V_n + T_n + E_n + A + A + a] = binomial_lpmf(T_A[a]|A_A[a], R_T[gear_A[a],section_A[a]]);  //Note: leaving ratio of cars per angler constant among days since was invariant!
	}
}