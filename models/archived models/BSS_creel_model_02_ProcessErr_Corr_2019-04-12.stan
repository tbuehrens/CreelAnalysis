...
model{
	...
	//Both
	for(p in 2:P_n){
		eps_C[p-1,] ~ multi_normal_cholesky(Zero, diag_pre_multiply(rep_vector(sigma_eps_C,G*S), Lcorr_C)); 
		eps_E[p-1,] ~ multi_normal_cholesky(Zero, diag_pre_multiply(rep_vector(sigma_eps_E,G*S), Lcorr_E)); 
	}
	for(g in 1:G){
		for(d in 1:D){
			for(s in 1:S){  					  
