data{
	...
	real prior_phi_C_prior; //the rate (alpha) and shape (beta) hyperparameters for the prior phi_C_prior ~ beta(prior_phi_C_prior,prior_phi_C_prior); XXX is an intermediate variable for phi_C
	real prior_phi_E_prior; //the rate (alpha) and shape (beta) hyperparameters for the prior phi_E_prior ~ beta(prior_phi_E_prior,prior_phi_E_prior); XXX is an intermediate variable for phi_E
...
transformed data{
	...
	vector[G*S] Zero; //vector used for process error correlation matrix
...
transformed parameters{
	...
	for(g in 1:G){
		...
		for(p in 2:P_n){
			for(s in 1:S){
				omega_C[s][p,g] = phi_C * omega_C[s][p-1,g] + to_matrix(eps_C[p-1,],G,S)[g,s]; 
				omega_E[s][p,g] = phi_E * omega_E[s][p-1,g] + to_matrix(eps_E[p-1,],G,S)[g,s]; 
			}													
		}
		for(d in 1:D){       
			for(s in 1:S){	
...
model{
	...
	//Both
	for(p in 2:P_n){
		eps_C[p-1,] ~ multi_normal_cholesky(Zero, diag_pre_multiply(rep_vector(sigma_C,G*S), Lcorr_C)); 
		eps_E[p-1,] ~ multi_normal_cholesky(Zero, diag_pre_multiply(rep_vector(sigma_E,G*S), Lcorr_E)); 
	}
	for(g in 1:G){
		for(d in 1:D){
			for(s in 1:S){  					  
